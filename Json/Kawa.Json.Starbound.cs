		...
		public static object Deserialize(BinaryReader stream)
		{
			...
			Func<object> @array = () =>
			{
				...
				if (!allAreNull)
				{
					...
					if (allAreSameType) return ret.Cast<string>().ToList();

					allAreSameType = true;
					foreach (var i in ret)
					{
						if (i == null) continue;
						if (!(i is int)) { allAreSameType = false; break; }
					}
					if (allAreSameType) return ret.Cast<int>().ToList();

					allAreSameType = true;
					foreach (var i in ret)
					{
						if (i == null) continue;
						if (!(i is double)) { allAreSameType = false; break; }
					}
					if (allAreSameType) return ret.Cast<double>().ToList();
				}
